...
if __name__ == '__main__':
    ...
    for mode in range(2):
        ...
        for simulation in range(simulations):
            ...
            #Append consumer hits for 95 confidence
            hit_c[mode].append([len(consumer.receivedPackets) for consumer in consumers.values()])
        ...
        confidence = 0.95
        hits_c[mode] = [sem(hit_c[mode][s]) * t.ppf((1 + confidence) / 2, len(hit_c[mode][s]) - 1)
    ...
    out_hit_c = pd.DataFrame(hits_c, index=consum)
    ...
    out_hit_c.columns = ['Ant routing', 'Flooding']
    ...
    out_hit_c = out_hit_c.sort_index()
    ...
    out_hit_c.to_csv('data/' + str(time.time())[:8] + 'hits_c.csv')
    out_waste.to_csv('data/' + str(time.time())[:8] + 'waste.csv')

    # Create figures
    # fig_hits2 = plt.figure()
    # fig_hits_a = plt.figure()
    fig_hits2 = plt.figure(figsize=[35, 25])
    fig_hits_a = plt.figure(figsize=[35, 20])
    ...
    ax.grid(axis='y')
    plot_hit_a = out_hit_a.plot.bar(yerr=out_hit_c, title="Content retrieved per consumer", ax=fig_hits_a.add_subplot(111))
    plot_hit_a.set(ylabel="Packets", xlabel='Consumers')
    plot_hit_a.grid(axis='y')
    plt.xticks(rotation=0)
